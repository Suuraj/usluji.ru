...
func OfferToUser(c *gin.Context) {
	...
	err := database.DB.QueryRow(c.Request.Context(), checkQuery, req.OrderID, myID).Scan(&exists)
	if err != nil || !exists {
	...
	_, err = database.DB.Exec(c.Request.Context(), query, req.OrderID, targetUserID)
	if err != nil {
